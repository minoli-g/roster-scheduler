...
const select_preference = async (req, res, next) => {
  ...
  Doctor.select_preference(doctor_id, (err, result) => {
    ...
    if (result.length > 0) {
      const sqlUpdate =
        "UPDATE `preferences` SET prefered_date=? WHERE `preferences`.`doctor_id` = ? AND preferences.month = MONTH(ADDDATE(CURRENT_DATE, INTERVAL 1 MONTH)) AND preferences.year = YEAR(ADDDATE(CURRENT_DATE, INTERVAL 1 MONTH))";
      ...
      const sqlInsert =
        "INSERT INTO `preferences` (`doctor_id`,year,month,prefered_date) VALUES (?,YEAR(ADDDATE(CURRENT_DATE, INTERVAL 1 MONTH)),MONTH(ADDDATE(CURRENT_DATE, INTERVAL 1 MONTH)),?)";
