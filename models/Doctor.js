...
class Doctor {
  ...
  static async send_report(doctor_id, date, message, callback) {
    const sqlInsert =
      "INSERT INTO `issue` (`doctor_id`, `message`, `date`) VALUES (?,?,?);";
  ...
  static async edit_profile(uname, fname, lname, uid, callback) {
    const sqlUpdate =
      "UPDATE `user` SET `username`=?,`first_name` = ?,`last_name`=? WHERE `user_id` = ?;";
  ...
  static async view_leave(token, userid, callback) {
    const sqlSelect =
      "SELECT DATE_FORMAT(STR_TO_DATE(date,'%Y-%m-%dT%H:%i:%s.000Z'),'%Y-%m-%d') AS `date`, `status` FROM `leave` WHERE `doctor_id` = ? ORDER BY date DESC";
  ...
  static async view_report(token, userid, callback) {
    const sqlSelect =
      "SELECT `message`,DATE_FORMAT(STR_TO_DATE(date,'%Y-%m-%dT%H:%i:%s.000Z'),'%Y-%m-%d') AS `date`, `status` FROM `issue` WHERE `doctor_id` = ? ORDER BY date DESC";
    await db.query(sqlSelect, [userid], (err, result) => {
      if (err) {
        callback(err, undefined);
      } else {
  ...
  static async roster(wardid, year, month, callback) {
    const sqlSelect =
      "SELECT `roster` FROM `roster` WHERE `ward_id`=? AND `year`=? AND `month`=?";
  ...
  static async doctor(user_id, callback) {
    const sqlSelect =
      "SELECT * FROM `user` WHERE `user_id`=? AND `type`='doctor'";
  ...
  static async work_hours(userid, callback) {
    const sqlSelect =
      "SELECT `month`,`work_hrs` FROM `working_hours` WHERE user_id=? ORDER BY workHour_id DESC LIMIT 12";
  ...
  static async getPre(userid, callback) {
    const sqlSelect =
      "SELECT DATE_FORMAT(STR_TO_DATE(prefered_date,'%Y-%m-%dT%H:%i:%s.000Z'),'%Y-%m-%d') AS `prefered_date` FROM `preferences` WHERE doctor_id = ? AND month = MONTH(ADDDATE(CURRENT_DATE, INTERVAL 1 MONTH)) AND year = YEAR(ADDDATE(CURRENT_DATE, INTERVAL 1 MONTH))";
  ...
  static async countPre(userid, callback) {
    const sqlSelect =
      "SELECT COUNT(*) FROM `preferences` WHERE doctor_id = ? AND month = MONTH(ADDDATE(CURRENT_DATE, INTERVAL 1 MONTH)) AND year = YEAR(ADDDATE(CURRENT_DATE, INTERVAL 1 MONTH))";
